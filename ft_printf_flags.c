...
/*   Updated: 2020/05/30 20:43:42 by apuchill         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_width(int *len, t_flags fl)
{
	...
	if (fl.pad_c == '0' && (fl.minus == 1 || fl.precision > -1) &&
...
static void	print_zeros(int *len, t_flags fl)
{
	...
	if (fl.spe_c == 'X' && fl.hash == 1 && fl.ulli != 0)
		ft_putcstr_len("0X", len, 2);
	if (fl.spe_c == 'o' && fl.hash == 1 && (fl.ulli != 0 ||fl.precision == 0))
...
void		print_flags(int *len, t_flags fl)
{
	...
	if (fl.ulli == 0 && fl.precision == 0 && fl.print_n0 == 1)
		fl.width++;
	if ((fl.spe_c == 'x' || fl.spe_c == 'X') && fl.hash == 1 && fl.ulli != 0)
		fl.width -= 2;
	if (fl.spe_c == 'o' && fl.hash == 1 && (fl.ulli != 0 ||fl.precision == 0))
		fl.width -= 1;
	if (fl.spe_c == 'o' && fl.hash == 1 && fl.ulli != 0 && fl.precision > -1)
		fl.precision -= 1;
	if (fl.precision > -1 && fl.spe_c != 'f')
		fl.pad_c = ' ';
